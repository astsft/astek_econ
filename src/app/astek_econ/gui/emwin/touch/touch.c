...
ui_touch_scan( void )
{
	...
	{
		Dev_Now.Touch = 0;
		gt9xx_x_read( GT9XX_REG_READ_XY, buf, 1 );


		if ((buf[0]&0x80) == 0x00) // check is data relevant;
		{
			gt9xx_x_write( GT9XX_REG_READ_XY, (uint8_t *)&Clearbuf, 1 );
		}
		else
		{
			...
			gt9xx_x_read( GT9XX_REG_READ_XY+1, &buf[1], Dev_Now.TouchCount*8 );
			gt9xx_x_write( GT9XX_REG_READ_XY, (uint8_t *)&Clearbuf, 1 );
...
static
void
ui_touch_get_id( void )
{
    uint8_t     buf[ 16]    = {0};


    gt9xx_x_read( GT9XX_REG_PRODUCT_ID, buf, 11 );
}
