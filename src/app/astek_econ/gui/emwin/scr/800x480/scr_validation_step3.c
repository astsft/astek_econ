...
init_dialog(                                            WM_HWIN         hWin )
{
    WM_HWIN         hItem;
    char            str[ 64];
    waiting_time = dev.validation.timings.return_time;
    ...
    if (dev.cfg.lang)      
      TEXT_SetText(           hItem, "ШАГ 3 ИЗ 3"  ); 
    else
      TEXT_SetText(           hItem, "STEP 3 FROM 3" ); 
...
scr_update(                             WM_HWIN         hWin)
{
  ...
  {  
    ...
    if (dev.validation.start == VALIDATION_STOP || dev.validation.start == VALIDATION_REMOTE_START)
    {
...
/**
  ...
  */
WM_HWIN
scr_validation_step3( void )
{
    send_cmd_for_validation_return_to_measure();
