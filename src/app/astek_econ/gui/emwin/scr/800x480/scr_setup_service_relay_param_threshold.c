...
dialog_callback(                                WM_MESSAGE *            pMsg )
{
    ...
    switch( pMsg->MsgId )
    {
            ...
            switch( WM_GetId( pMsg->hWinSrc ) )
            {
                ...
                case GUI_ID_BUTTON_DUMMY:
                    if( pMsg->Data.v == WM_NOTIFICATION_RELEASED )
                    {
                        ...
                        level  = 0;
                        level  += 100000 * ((symb_countof - 1) - LISTWHEEL_GetPos( WM_GetDialogItem( pMsg->hWin, GUI_ID_LISTWHEEL1 )));
                        level  += 10000  * ((symb_countof - 1) - LISTWHEEL_GetPos( WM_GetDialogItem( pMsg->hWin, GUI_ID_LISTWHEEL2 )));
                        level  += 1000   * ((symb_countof - 1) - LISTWHEEL_GetPos( WM_GetDialogItem( pMsg->hWin, GUI_ID_LISTWHEEL3 )));
                        level  += 100    * ((symb_countof - 1) - LISTWHEEL_GetPos( WM_GetDialogItem( pMsg->hWin, GUI_ID_LISTWHEEL4 )));                                                                    
                        if (dev.gui.scr_idx == SCR_IDX_SETUP_SERVICE_RELAY_1_PARAM)
                        {
                          dev.ext_relay->relay[EXTERNAL_1].thld_type = thld_type;                          
                          dev.ext_relay->relay[EXTERNAL_1].ppm.ppm_f = level;                                 
                          dev.ext_relay->relay[EXTERNAL_1].hyst_ppm.ppm_f = (level * 5) / 100;
                          if (dev.ext_relay->relay[EXTERNAL_1].ppm.ppm_f == 0 && dev.ext_relay->relay[EXTERNAL_1].hyst_ppm.ppm_f == 0) dev.ext_relay->relay[EXTERNAL_1].hyst_ppm.ppm_f = 0.1;
                          convert_flt_to_int_fract(dev.ext_relay->relay[EXTERNAL_1].ppm.ppm_f, &dev.ext_relay->relay[EXTERNAL_1].ppm.integral.i32, &dev.ext_relay->relay[EXTERNAL_1].ppm.fractional.i32);                                                    
                          relay_config =  dev.ext_relay->relay[EXTERNAL_1].relay_mode << 16;
                          relay_config |= dev.ext_relay->relay[EXTERNAL_1].relay_state << 8;
                          relay_config |= dev.ext_relay->relay[EXTERNAL_1].thld_type;
                          send_cmd_for_nvm_write_param(NVM_REG_RELAY1_MODE_STATE_TYPE, relay_config);                           
                          send_cmd_for_nvm_write_param(NVM_REG_RELAY1_THRESHOLD, (uint32_t)dev.ext_relay->relay[EXTERNAL_1].ppm.ppm_f);
                        }
                        else
                        {
                          dev.ext_relay->relay[EXTERNAL_2].thld_type = thld_type;                          
                          dev.ext_relay->relay[EXTERNAL_2].ppm.ppm_f = level;                                 
                          dev.ext_relay->relay[EXTERNAL_2].hyst_ppm.ppm_f = (level * 5) / 100;
                          if (dev.ext_relay->relay[EXTERNAL_2].ppm.ppm_f == 0 && dev.ext_relay->relay[EXTERNAL_2].hyst_ppm.ppm_f == 0) dev.ext_relay->relay[EXTERNAL_2].hyst_ppm.ppm_f = 0.1;
                          convert_flt_to_int_fract(dev.ext_relay->relay[EXTERNAL_2].ppm.ppm_f, &dev.ext_relay->relay[EXTERNAL_2].ppm.integral.i32, &dev.ext_relay->relay[EXTERNAL_2].ppm.fractional.i32);                                                   
                          relay_config =  dev.ext_relay->relay[EXTERNAL_2].relay_mode << 16;
                          relay_config |= dev.ext_relay->relay[EXTERNAL_2].relay_state << 8;
                          relay_config |= dev.ext_relay->relay[EXTERNAL_2].thld_type;
                          send_cmd_for_nvm_write_param(NVM_REG_RELAY2_MODE_STATE_TYPE, relay_config); 
                          send_cmd_for_nvm_write_param(NVM_REG_RELAY2_THRESHOLD, (uint32_t)dev.ext_relay->relay[EXTERNAL_2].ppm.ppm_f);
                        }
        ...
        case WM_INIT_DIALOG:
            ...
            if (dev.gui.scr_idx == SCR_IDX_SETUP_SERVICE_RELAY_1_PARAM)
            {
              level = dev.ext_relay->relay[EXTERNAL_1].ppm.ppm_f;
              thld_type = dev.ext_relay->relay[EXTERNAL_1].thld_type;
            }
            else
            {
              thld_type = dev.ext_relay->relay[EXTERNAL_2].thld_type;
              level = dev.ext_relay->relay[EXTERNAL_2].ppm.ppm_f;;           
            }
            ...
            idx     = (symb_countof - 1) - ((int32_t)level / 100000) % 10;
            LISTWHEEL_SetPos( hItem, idx );
            LISTWHEEL_SetSel( hItem, idx );
            ...
            idx     = (symb_countof - 1) - ((int32_t)level /  10000) % 10;
            LISTWHEEL_SetPos( hItem, idx );
            LISTWHEEL_SetSel( hItem, idx );
            ...
            idx     = (symb_countof - 1) - ((int32_t)level /   1000) % 10;
            LISTWHEEL_SetPos( hItem, idx );
            LISTWHEEL_SetSel( hItem, idx );
            ...
            idx     = (symb_countof - 1) - ((int32_t)level /    100) % 10;
            LISTWHEEL_SetPos( hItem, idx );
            LISTWHEEL_SetSel( hItem, idx );
