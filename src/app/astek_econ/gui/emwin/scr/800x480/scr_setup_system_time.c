...
dialog_callback(                        WM_MESSAGE *            pMsg )
{
    ...
    switch( pMsg->MsgId )
    {
        ...
        case WM_NOTIFY_PARENT:
            switch( WM_GetId( pMsg->hWinSrc ) )
            {
                ...
                case GUI_ID_BUTTON_DUMMY:
                    if( pMsg->Data.v == WM_NOTIFICATION_RELEASED )
                    {
                        idx_hour    = LISTWHEEL_GetPos( WM_GetDialogItem( pMsg->hWin, GUI_ID_LISTWHEEL0 ) );
                        idx_minute  = LISTWHEEL_GetPos( WM_GetDialogItem( pMsg->hWin, GUI_ID_LISTWHEEL1 ) );
                        idx_second  = LISTWHEEL_GetPos( WM_GetDialogItem( pMsg->hWin, GUI_ID_LISTWHEEL2 ) );
        ...
        case WM_INIT_DIALOG:
            ...
            gui_init_listwheel( pMsg->hWin, GUI_ID_LISTWHEEL2, 40, seconds, sizeof( seconds ) / sizeof( char * ) );
            ...
            gui_init_listwheel( pMsg->hWin, GUI_ID_LISTWHEEL1, 40, minutes, sizeof( minutes ) / sizeof( char * ) );
            idx     = dev.mcu->rtc.get_minute();
            LISTWHEEL_SetPos( hItem, idx );
            LISTWHEEL_SetSel( hItem, idx );

            hItem   = WM_GetDialogItem( pMsg->hWin, GUI_ID_LISTWHEEL0 );
            gui_init_listwheel( pMsg->hWin, GUI_ID_LISTWHEEL0, 40, hours, sizeof( hours ) / sizeof( char * ) );
